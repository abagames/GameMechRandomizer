declare module SAT {
    class Vector {
        ...
        isIn(spacing?: number, minX?: number, maxX?: number,
            minY?: number, maxY?: number): boolean;
        angleTo(other: Vector): number;
        distanceTo(other: Vector): number;
        moveTo(target: Vector, speed: number): Vector;
        moveAngle(angle: number, speed: number): Vector;
    }
    class Circle {
        pos: Vector;
